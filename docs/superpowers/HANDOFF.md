...
## Where the project stands (as of 2026-05-12 — v1.5 + 2 papercuts fixed; Tier 4 dogfood pause is the gating next step)

main is at tag `v0.70.0-tier-3-papercut-bundle` (HEAD `b4ae473`). **Phases 1-11 ✅ ALL COMPLETE for v1.** Phase 6 closed at 11/11 verbs; Phase 7 closed at 5/5 (capture pipeline); Phase 8 closed at 4/4 (obscura adapter); Phase 9 closed at 5/5 (flow runner); **Phase 10 closed at 3/3 sub-parts (schema migration tooling)**; Phase 11 closed at 5/5 sub-parts (memory cache); v1-polish bundle SHIPPED (PR #107); cache-write-security recipe SHIPPED (PR #94); selector-mode plumbing 3/4 + playwright-lib `--selector` SHIPPED (PRs #99/#101/#103/#105; press deferred per SS5). **Doctor refresh SHIPPED (PR #113)** — pending-migrations warn + memory cache hit-rate read side. **Phase 11 v2 part 1 SHIPPED (PR #115; Pick A1)** — `events.jsonl` writer tees per-invocation observations from `browser-do --intent`. **Pick D SHIPPED (PR #117)** — SKILL.md verb table gained `browser-migrate` rows + "Migration & schema evolution" section; README.md verb count corrected (41→42); doctor `n/a` text refreshed (was "Phase 11 v2 pending"; now "no events yet — run `browser-do --intent`..."); new `tests/docs-coverage.bats` pins hand-curated doc rows against silent drift. **End-to-end ROI loop live:** `browser-do --intent` → `events.jsonl` → `browser-doctor.sh` reports real hit-rate. **Pick A5 SHIPPED (PR #119)** — `self_heal_history[]` audit trail populates on enabled→disabled and disabled→enabled (D2 heal) transitions; schema field reserved since Phase 11 1-i, populated now. **Pick A3 SHIPPED (PR #121)** — `--auto-record` flag on `browser-do propose` batch-persists proposed patterns via `memory_record_pattern`; idempotent across runs (suppression filter pre-filters known); summary line gains `auto_recorded:N` field. **Pick A4 SHIPPED (PR #123)** — pattern-equivalence canonicalization (`:NAME` → `:_` collapse) for compares in `memory_record_pattern` idempotency check + `browser-do propose` suppression filter; resolver was already param-agnostic by design; storage preserves original `:NAME` for readability; first-write wins on canonical match. **Pick A6 SHIPPED (PR #125)** — passive navigation observation log `recent_urls.jsonl` (mode 0600 in mode 0700 memory/); `browser-open` tees URLs on success; `browser-do propose --from-recent` consumes the log; new write site = `browser-open` only (smallest reviewable surface; future verbs tee in follow-ups). **Pick A2 SHIPPED (PR #127) — closes Phase 11 v2 hardening backlog** — slug heuristic (`SLUG_RE` + `MIN_SLUG_LEN=5`) in `url-pattern-cluster.mjs` clusters `/posts/my-post`-style segments to `:slug`; ordered numeric → UUID → slug → literal; rejects short codes (`a-b`) + single-word literals (`about`). **Pick B SHIPPED (PR #129) — closes ALL HANDOFF-locked PR picks** — daemon e2e for `playwright-lib --selector` (2 cases: click round-trip + secret-scrub regression on selector branch); gated behind real Playwright install via existing `setup_file()` pattern. PR #105's runtime branches now CI-gated against silent regression. **Stage 4 part 1 SHIPPED (PR #131)** — 4 agent-workflow recipes (`login-then-scrape` + `incremental-pattern-discovery` + `flow-record-and-replay` + `cache-driven-bulk-operation`) tutorial-shaped walkthroughs; SKILL.md gains "Agent-workflow recipes" section; `tests/docs-coverage.bats` gains 3 drift-guard cases pinning recipe presence + verb-name references. Total recipes: 7 pattern + 4 workflow = 11. **Tier 3 papercut bundle SHIPPED (PR #133)** — doctor `check:"recent_urls"` block (line count + JSON event, parallel to `check:"memory_cache"`) + `fill --selector` short-timeout (default 5s; `BROWSER_SKILL_FILL_TIMEOUT_MS` env override; the test case dropped from PR #129 now passes). 30 LOC total + 3 bats; two long-deferred items bundled into one cycle per the "bundle-with-next-touching-area" annotation pattern.
...
| Pick A2 (slug heuristic — closes Phase 11 v2) | `scripts/lib/node/url-pattern-cluster.mjs` gains `SLUG_RE = /^[a-z0-9_]+(-[a-z0-9_]+)+$/i` + `MIN_SLUG_LEN = 5`. Fires after numeric/UUID in the if-chain order. Templates qualifying segments to `:slug` → cluster + propose. Rejects short codes (`a-b`/`1-2`) and single-word literals (`about`/`login`). Accepts dates (`2025-01-15`) and slugs (`alice-cooper`). ~6 LOC in helper + 4 bats (1 flipped from "slugs don't cluster" → "slugs DO cluster"). Composes naturally with A3 `--auto-record` + A6 `--from-recent` (both consume cluster output). | ✅ (PR #127) |
| Pick B (daemon e2e for --selector — closes all PR picks) | `tests/playwright-lib_stateful_e2e.bats` gains 2 cases exercising PR #105's previously-untested IPC handler selector branches: (1) `click --selector h1` end-to-end (asserts `event:"click"`, `status:"ok"`, AND `.ref == null` — proves selector branch took); (2) `fill --selector --secret-stdin` secret-scrub regression for the selector branch. Both gated behind real Playwright install via existing `setup_file()` pattern. Pure test addition (0 production code change). PR #105's runtime branches now CI-gated. | ✅ (PR #129) |
| Stage 4 part 1 (agent-workflow recipes) | New `references/recipes/agent-workflows/` subdirectory + README index + 4 tutorial-shaped recipes (`login-then-scrape`, `incremental-pattern-discovery`, `flow-record-and-replay`, `cache-driven-bulk-operation`). Distinct from pattern recipes (codified discipline) — workflow recipes show sequenced commands + expected output for actual user-facing tasks. SKILL.md gains "Agent-workflow recipes" section linking all 4. Design decision: clean-state assumption — recipes walk through full setup from fresh `~/.browser-skill/`. Reproducibility > brevity. 3 docs-coverage.bats drift-guards (README index links + recipe presence + verb-name references). +550 LOC across 5 md files + 1 bats. | ✅ (PR #131) |
| Tier 3 papercut bundle (doctor recent_urls + fill-selector timeout) | Two deferred items bundled per HANDOFF "bundle-with-next-touching-area" annotation. (1) Doctor `check:"recent_urls"` block — `browser-doctor.sh` reports `recent_urls.jsonl` line count + emits JSON event parallel to `check:"memory_cache"` shape. Absent log → `count:0`; present → `count:N`. (2) `fill --selector` short-timeout — `playwright-driver.mjs::case 'fill'` selector path now passes `timeout: 5000` (env override `BROWSER_SKILL_FILL_TIMEOUT_MS`) to `locator.fill()`. Default Playwright locator timeout (30s) was blocking the daemon on no-match selectors; the test case dropped from PR #129 now passes within 15s budget. 30 LOC + 3 bats. | ✅ (PR #133) |
...
- **977 tests pass / 0 fail / lint exit 0** locally (974 prior + 3 from PR #133 Tier 3 bundle: 2 doctor recent_urls + 1 selector short-timeout).
- **130 PRs merged total** (... + Stage 4 part 1 (PR #131) + **Tier 3 papercut bundle (PR #133)**; 30 HANDOFF refreshes; not counting this HANDOFF refresh).
- **11 recipes shipped:** 7 pattern (codified discipline) + 4 workflow (tutorial walkthroughs).
- **Doctor advisory blocks: 3** — `migrations` (PR #113) · `memory_cache` (PR #113 read; PR #115 writer) · `recent_urls` (PR #133).
...
## Next session: Tier 4 7-day dogfood pause (HIGHEST ROI; gating step; no PR)

**All bundled-PR-mode work shipped.** PR cadence is now low-ROI per LOC — subsequent small PRs without dogfood data are educated guesses. The highest-leverage next move is **not a PR**.

**Tier 4 — 7-day dogfood pause (RECOMMENDED; the actual gating step):**
- Pick a real personal/work site (task tracker, billing dashboard, vendor portal — anything you use daily).
- Use the toolchain daily for 1 week. Walk through agent-workflow recipes; iterate on real tasks; let `events.jsonl` + `recent_urls.jsonl` accumulate.
- **Measure:**
  - Cache hit rate after 20+ same-archetype actions (design target ≥70%; useful threshold ≥40%; below that, heuristics need tuning).
  - Wall-clock per action (cache hit vs miss).
  - Token cost via `claude -p` harness (cache hit ≈ <500 input tokens; miss ≈ 3-10K).
- **Findings shape subsequent work.** Possible: heuristic tweaks (slug too narrow? semantic similarity for intent matching?), adapter overhead (cdt-mcp 5-20s snapshot — optimize?), UX papercuts (which `die()` messages need better next-action hints?).

**Why dogfood now (not more PRs):**
- All advertised features SHIP-tested. Tests pass at 977/0 — code is correct.
- No advertised feature is unused by tests. Coverage is dense.
- What's UNKNOWN is real-world ROI. The toolchain claims ≥70% hit rate; no one has verified it under load.
- Each additional small PR has diminishing returns. The next dataset-driven PR will be 10x more targeted than a speculative one.

**Tier 2 — Distribution polish (non-PR research; 1-2 PRs):**
- README hero refresh + GIF demo / asciinema cast (~15s). Needs real recording; not a PR-shaped task.
- GitHub repo metadata: description, topics tags, social preview image. Pure web-form changes; no code PR.
- Claude Code skills marketplace listing (if exists). Research first; file issue with Anthropic if needed.

**Tier 5 — Long-tail maintenance (background; ongoing):**
- Pin upstream adapter versions in CI (currently `@latest`).
- Real-mode CI gating — expand Playwright-gated tests to GitHub Actions runner.
- Periodic schema-migration drills (quarterly).
- Linux parity dogfood check.

**Speculative tiny bundle candidates (avoid until dogfood confirms need):**
- `browser-doctor --fix` — auto-remediate `chmod 700` perms, lazy-create missing dirs.
- Better error messages with next-action hints in every `die()`.
- `browser-init` wizard — interactive: site → creds → first session.
- `click --selector` short-timeout (parallel to PR #133's `fill`).

**Recommended sequence:**
1. **STOP PR mode.** Dogfood for 7 days on a real site. Collect numbers.
2. Tier 2 distribution polish — non-PR mostly; 1 PR for README/GIF.
3. Tier 3 papercut PRs (only those validated by dogfood findings).
4. Tier 5 maintenance hygiene as ongoing.

**Phase ordering recap:**
- All v1 phases ✅ COMPLETE
- All Phase 11 v2 hardening items ✅ SHIPPED
- All HANDOFF-locked PR picks ✅ SHIPPED (C/A1-A6/D/B)
- Stage 4 part 1 ✅ SHIPPED (PR #131) — agent-workflow recipes
- **Tier 3 papercut bundle ✅ SHIPPED (PR #133)** — doctor recent_urls + fill-selector timeout
- **Tier 4 7-day dogfood pause 🔲 — GATING STEP (recommended next)**
- Tier 2 distribution polish 🔲 — non-PR research + 1-2 PRs
- Tier 5 long-tail maintenance 🔲 — background ongoing
- Speculative Tier 3 papercuts 🔲 — defer until dogfood confirms need
...
- **Doctor refresh ✅ SHIPPED (PR #113)** — Pick C
- **Phase 11 v2 part 1 ✅ SHIPPED (PR #115)** — Pick A1, events.jsonl writer
- **Pick D ✅ SHIPPED (PR #117)** — browser-migrate doc refresh + docs-coverage.bats
- **Pick A5 ✅ SHIPPED (PR #119)** — self_heal_history[] audit trail
- **Pick A3 ✅ SHIPPED (PR #121)** — `--auto-record` flag
- **Pick A4 ✅ SHIPPED (PR #123)** — pattern-equivalence canonicalization
- **Pick A6 ✅ SHIPPED (PR #125)** — recent_urls.jsonl observation log
- **Pick A2 ✅ SHIPPED (PR #127)** — slug heuristic; closes Phase 11 v2 hardening backlog
- **Pick B ✅ SHIPPED (PR #129)** — daemon e2e for --selector; closes all PR picks
- **All HANDOFF-locked PR work ✅ COMPLETE. Project is production-ready v1.5.**
- Stage 4 adoption (recipes / demo / cookbook / blog) 🔲 — RECOMMENDED
- Tiny bundle candidates: doctor recent_urls surface · `fill --selector` short-timeout — fold into next-touching-area PRs.
...
- **Post-merge tag-placement pattern** (codified in PR #113; proven 11× clean first-try in PRs #115, #117, #119, #121, #123, #125, #127, #129, #131, #133): GitHub squash-merges create a brand new commit on main; the pre-squash branch tip is orphaned (still reachable by tag, but not from main). **Tag AFTER the squash-merge, on the new main commit** — not before the merge on the branch tip. Local workflow: `gh pr merge --squash` → `git checkout main && git pull` → `git tag vX.Y.Z $(git rev-parse HEAD)` → `git push origin vX.Y.Z`. Confirmed by precedent: 11 consecutive clean first-try placements (v0.59.0 through v0.70.0). **Load-bearing for every substantive PR.**
- **Bundle-with-next-touching-area pattern** (codified in PR #133): when small deferred items accumulate in HANDOFF's "bundle with next touching area" list, ship them as a bundled PR when one item's area is touched, rather than as separate per-item PRs. PR #133 shipped 2 such items (doctor recent_urls + fill-selector timeout) at 30 LOC total instead of 2× 15 LOC PRs. Pattern: HANDOFF maintains a running list of "fold into next-touching-area" items with explicit area annotations; when about to ship in that area, sweep the list. Decision rubric: only fold items <25 LOC each AND in the same touching surface; large items get their own PR even if "deferred." Caps bundle at ~3 items / ~60 LOC to keep review tractable.
- **Branch-before-commit safety pattern** (codified retroactively in PR #129 recovery): committing substantive work directly to main locally — then trying to push — fails because GH expects a feature branch. Recovery: `git branch feat/X` → `git push -u origin feat/X` → `git checkout main && git reset --hard origin/main` (commit preserved on the new branch; main restored). Mild gotcha; one-line fix; no rework on the PR itself. Decision rubric: **always `git checkout -b BRANCH-NAME` BEFORE the first `git commit` of substantive work**; check `git branch --show-current` if unsure. Caught once in PR #129; codified to save next time.
...
2. Confirm tag is `v0.70.0-tier-3-papercut-bundle` and main HEAD matches `b4ae473`.
3. Read CHANGELOG `[Unreleased]` block — Tier 3 papercut bundle (PR #133) is the top entry; Stage 4 part 1 (PR #131) second.
4. **PR-mode is at end-of-runway.** All bundled work shipped. **Continuing into more PRs without dogfood data is low-ROI.** The honest framing: 977 tests pass; code is correct; what's UNKNOWN is real-world ROI under load.
5. **RECOMMENDED (highest-ROI; not a PR):** **7-day dogfood on a real site.** Walk the agent-workflow recipes (`references/recipes/agent-workflows/`) against your task tracker / billing dashboard / vendor portal. Let `events.jsonl` + `recent_urls.jsonl` accumulate. Measure:
   - Cache hit rate after 20+ same-archetype actions: target ≥70%; useful ≥40%.
   - Wall-clock per action (cache hit vs miss).
   - Token cost (cache hit ~<500 tokens; miss ~3-10K).
6. **Alternative (if explicit user direction; not auto-pilot):** Tier 2 distribution polish — README hero + asciinema cast + GitHub topics. Mostly non-PR; 1 PR for README/GIF.
7. **Speculative tiny bundle candidates (DEFER until dogfood):** `browser-doctor --fix` · better `die()` messages with next-action hints · `browser-init` wizard · `click --selector` short-timeout. **Do not ship these speculatively** — let dogfood findings tell you which papercuts actually matter.

Start with: read CHANGELOG `[Unreleased]` block since `v0.70.0-tier-3-papercut-bundle` to confirm no in-flight work, then **propose pausing PR-mode for dogfood**. If user wants to continue in PR-mode anyway, surface the speculative candidates above but flag that each has diminishing returns.

**Reading priority IF user opts to continue PR-mode (against recommendation):**
1. `references/recipes/agent-workflows/` — read all 4 recipes. They ARE the dogfood script. Each step is reproducible from a fresh `~/.browser-skill/`.
2. `scripts/browser-doctor.sh` — for any "doctor improves" speculation, this is the touch surface. Existing 3 check-blocks (migrations / memory_cache / recent_urls) are the templates.
3. `scripts/lib/common.sh::die` — for "better error messages" speculation. Every die() call site is a candidate for next-action hints.
4. `docs/superpowers/HANDOFF.md` workflow-patterns section — 11+ codified patterns to consult before any new PR. The "bundle-with-next-touching-area" pattern is especially relevant for any small cleanup work.
5. **Open question (don't pre-answer; ask the user):** "Want to continue shipping speculative papercut PRs, or pause for dogfood?" Frame honestly that the latter is higher-ROI but the former is faster-feeling.
