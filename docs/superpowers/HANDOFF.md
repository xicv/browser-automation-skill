...
- **974 tests pass / 0 fail / lint exit 0** locally (971 prior + 3 docs-coverage drift-guards from PR #131 Stage 4 part 1).
- **128 PRs merged total** (... + Pick B daemon e2e — closes all PR picks (PR #129) + **Stage 4 part 1 agent-workflow recipes (PR #131)**; 29 HANDOFF refreshes; not counting this HANDOFF refresh).
...
## Next session: Tier 3 papercut bundle OR Tier 2 distribution polish OR 7-day dogfood pause (highest ROI; non-PR)

**Stage 4 part 1 ✅ SHIPPED (PR #131)** — 4 agent-workflow recipes. All PR-mode pick lists closed; remaining work splits into Tiers per the 5-tier roadmap from the prior session.

**Tier 3 papercut bundle (PROMOTED — RECOMMENDED for next PR):**
- **Doctor `recent_urls` surface** — extend `browser-doctor.sh` with `check:"recent_urls"` block (count + warn if mode != 0600). ~15 LOC + ~2 bats. Bundles with existing `check:"memory_cache"` shape (PR #113).
- **`fill --selector` short-timeout** — PR #129's deferred bug. Driver passes shorter timeout to `locator.fill()` so `--selector` no-match returns quickly instead of blocking 30s. ~10 LOC in `playwright-driver.mjs` + ~1 bats (the test case dropped from PR #129 can now pass).
- **Bundle**: ~30 LOC total + ~3 bats. Single small PR; both items flagged in HANDOFF for "bundle with next touching area" — ship together.

**Tier 2 distribution polish (non-trivial; non-PR ideal):**
- README hero section refresh + GIF demo (~15s asciinema cast). Hard to ship via PR alone; needs a real recording.
- GitHub repo metadata: description, topics, social preview. Pure web-form changes; no code PR.
- Marketplace listing (if Claude Code skills marketplace exists; research needed). Non-PR.

**Tier 4 — 7-day dogfood pause (HIGHEST ROI; no PR estimate):**
- Pick a real personal/work site. Use the toolchain daily for a week. Measure cache hit rate (target ≥40%, design claim ≥70%), wall-clock vs raw Playwright, token cost via `claude -p` harness.
- **This is the gating step.** Without dogfood, all Tier 3 papercut fixes are educated guesses. With it, you'll know which corners matter.
- Possible findings: heuristic adjustments (slug too narrow? add edge cases? add semantic similarity for intent matching?); adapter overhead (cdt-mcp 5-20s on snapshot — needs optimization?); UX papercuts (which die() messages need better next-action hints?).
...
- Periodic schema-migration drills.
- Linux parity dogfood check.

**Recommended sequence:**
1. Ship Tier 3 papercut bundle as a small PR (~30 LOC). Closes 2 known papercuts cleanly.
2. **Pause PR-mode + dogfood for 7 days** on a real site. Measure ROI.
3. Based on dogfood findings: 2-5 more targeted Tier 3 PRs.
4. Tier 2 distribution polish (non-PR research + 1-2 PRs).
5. Tier 5 maintenance hygiene as ongoing.

**Phase ordering recap:**
- All v1 phases ✅ COMPLETE
- All Phase 11 v2 hardening items ✅ SHIPPED (PRs #115/119/121/123/125/127)
- All HANDOFF-locked PR picks ✅ SHIPPED (PRs #113-#129)
- **Stage 4 part 1 ✅ SHIPPED (PR #131)** — agent-workflow recipes
- Tier 3 papercut bundle 🔲 — RECOMMENDED (doctor recent_urls + fill-selector timeout)
- Tier 4 7-day dogfood 🔲 — gating step before further iteration
- Tier 2 distribution polish 🔲 — non-PR research + 1-2 PRs
- Tier 5 long-tail maintenance 🔲 — background
...
- **Post-merge tag-placement pattern** (codified in PR #113; proven 9× clean first-try in PRs #115, #117, #119, #121, #123, #125, #127, #129): GitHub squash-merges create a brand new commit on main; the pre-squash branch tip is orphaned (still reachable by tag, but not from main). **Tag AFTER the squash-merge, on the new main commit** — not before the merge on the branch tip. Local workflow: `gh pr merge --squash` → `git checkout main && git pull` → `git tag vX.Y.Z $(git rev-parse HEAD)` → `git push origin vX.Y.Z`. If tag was placed pre-squash, fix with: `git tag -d TAG && git push origin :refs/tags/TAG && git tag TAG <squash-merge-sha> && git push origin TAG`. Confirmed by precedent: `v0.59.0-...` at `1f5be98` (PR #111); `v0.61.0-...` at `ea7e8ed` (PR #115); `v0.62.0-...` at `5ee375e` (PR #117); `v0.63.0-...` at `ee678af` (PR #119); `v0.64.0-...` at `f3905be` (PR #121); `v0.65.0-...` at `53df764` (PR #123); `v0.66.0-...` at `40b9d94` (PR #125); `v0.67.0-...` at `804e1f0` (PR #127); `v0.68.0-...` at `25d7b63` (PR #129) — all placed first-try post-merge, no rework. **Load-bearing for every substantive PR.**
...
2. Confirm tag is `v0.69.0-stage-4-agent-workflow-recipes` and main HEAD matches `f0493b3`.
3. Read CHANGELOG `[Unreleased]` block — Stage 4 part 1 (PR #131) is the top entry; Pick B (PR #129) second.
4. **Recommended:** **Tier 3 papercut bundle** — small PR combining (a) doctor `recent_urls` surface (`check:"recent_urls"` block in `browser-doctor.sh`, ~15 LOC + 2 bats) + (b) `fill --selector` short-timeout (driver passes shorter timeout to `locator.fill()`; ~10 LOC + 1 bats — the test case dropped from PR #129 can now pass). Both flagged in HANDOFF for bundle-with-next-touching-area; ship together. ~30 LOC + ~3 bats. Smallest reviewable PR.
5. **Alternative (gating step; no PR):** **7-day dogfood on a real site.** Measure cache hit rate (target ≥40%; design claim ≥70%), wall-clock vs raw Playwright, token cost. Findings shape subsequent Tier 3 PRs. **This is the highest-ROI activity left.**
6. **Alternative (non-PR research):** Tier 2 distribution polish — README hero refresh + asciinema cast + GitHub topics tags + marketplace research.
7. **Dogfood pause (optional verification, no PR):** the full v1.5 toolchain is live. Walk through any of the 4 agent-workflow recipes at `references/recipes/agent-workflows/` on a real site you actually use. The recipes are the dogfood script.

Start with: read CHANGELOG `[Unreleased]` block since `v0.69.0-stage-4-agent-workflow-recipes` to confirm no in-flight work, then propose Tier 3 papercut bundle sub-scope (or alternative). User prefers "go for your recommendation" once the option-table is presented; default to the smallest reviewable PR delivering user-visible value.

**Reading priority for Tier 3 papercut bundle:**
1. `scripts/browser-doctor.sh` — find the `check:"memory_cache"` block (PR #113). The new `check:"recent_urls"` block mirrors its shape: count lines, mode check, JSON event + ok/warn line.
2. `tests/doctor.bats` — existing `check:"memory_cache"` bats are the template for the new `check:"recent_urls"` cases.
3. `scripts/lib/node/playwright-driver.mjs` — `runFill` function (~line 126). Pass `timeout: 5000` (or env-overrideable) to `locator(selector).fill(text, { timeout })`. The default 30s blocks the daemon under no-match selector.
4. `tests/playwright-lib_stateful_e2e.bats` — the test case dropped from PR #129 (`fill --selector 'input[name="missing"]'` returns well-formed error event). After short-timeout, this should pass within ~5s.
5. **Decision lock (do before code):** `BROWSER_SKILL_FILL_TIMEOUT_MS` env override defaults to 5000? Or hardcode? Probably env-overridable for tests that need longer; default 5000 covers 99% of cases.
